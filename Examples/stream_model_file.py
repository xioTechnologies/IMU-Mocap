...
calibraiton_pose = imumocap.get_pose(root)
...
while True:
    ...
    if any([i.button_pressed for i in imus.values()]):
        print("Please hold the calibraiton pose")

        time.sleep(2)

        imumocap.solvers.calibrate(root, {n: i.matrix for n, i in imus.items()}, calibraiton_pose)
