...
for y in [np.sin(x) for x in np.linspace(0, np.pi, 100)]:
    ...
    model.joints["Left Wheel"].set(y * 360)
    model.joints["Right Wheel"].set(y * 360)

    model.joints["Seat"].set(tilt=y * 30)  # root joint connects the model to the world

    imumocap.solvers.translate(model.root, [y * 2 * np.pi * model.left_wheel.length, 0, 0])
