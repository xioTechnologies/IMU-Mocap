        ...
        Pass
        {
           ...
           CBUFFER_START(UnityPerMaterial)
            ...
            Varyings vert(Attributes input)
            {
                ...
                output.color.a = 1.0;
                output.uva = float3(input.uv.x, input.uv.y, _NearColor.a);
            ...
            {
                // sample texture
                //float4 texColor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, input.uva.xy);

                clip(input.uva.y - input.uva.z);
                
                // return float4(input.uva.y, input.uva.y, input.uva.y, 1); // texColor; // float4(input.uva.y, input.uva.y, input.uva.y > input.uva.z ? 0 : 1, 1);
                
                // if (input.uva.y > input.uva.z)
                // {
                //     discard;
                // }
                return input.color;
