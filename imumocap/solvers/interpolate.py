...
def interpolate(links: list[Link]) -> None:
    ...
    for index, _ in enumerate(links[:-1]):
        if links[index + 1] not in [l for l, _ in links[index].links]:
            raise ValueError(f"The links are not a contigious chain. {links[index + 1].name} is not connected to {links[index].name}.")

    for link in links[1:-1]:
        link.joint = Matrix()

    matrices = Matrix.slerp(links[0].get_joint_global(), links[-1].get_joint_global(), len(links))

    for link, matrix in zip(links, matrices):
        link.set_joint_global(matrix)
